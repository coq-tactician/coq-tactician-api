...
struct Graph {
  ...
  struct Node { # Fits exactly in 128 bits.
    ...
    # identity are merged when the graph is generated. Nodes are only merged when the first node exists in the
    # same graph as the second node, or exists in a dependent graph. Hence, nodes originating from
    # developments that do not depend on each other are never merged. Full graph-sharing would require
    # global analysis on a dataset, which any consumer can optionally do as a post-processing step.
    #
