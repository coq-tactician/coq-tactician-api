...
let populate_global_context_info tacs env ctacs cgraph cdefinitions =
  let { paths=_; def_count; node_count; edge_count; defs; nodes; edges }, state =
    ...
    let (state, _), builder =
      CICGraphMonad.run_empty ~include_opaque:false ~def_truncate:(truncate_option ()) updater G.builder_nil Global in
    builder, state in
...
module NeuralLearner : TacticianOnlineLearnerType = functor (TS : TacticianStructures) -> struct
  ...
  let predict rc wc find_global_argument state tacs env ps =
    ...
    let (_, (root, context_map)), { paths=_; def_count; node_count; edge_count; defs; nodes; edges } =
      CICGraphMonad.run ~include_opaque:false ~state updater G.builder_nil Local in
    let node_local_index (_, (def, i)) =
      if def then i else def_count + i in
